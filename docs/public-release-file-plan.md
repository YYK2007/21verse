...
- `include_pending_final_review`: tracked Unity asset files under retained project folders. These remain visible for final review before publication.
- `exclude_until_resolved`: tracked Unity files under asset folders that require rights confirmation, replacement, external import, or removal before public release.
- `exclude_generated`: generated outputs that should not be part of a public source tree.

This plan is a staging aid, not a release approval. The repository must stay private until the release audit has no content blockers and publication is explicitly approved.
